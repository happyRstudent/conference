...
async function fetchOpenAlexRecentWorks(authorId: string): Promise<string[]> {
  ...
  const url = `https://api.openalex.org/works?filter=authorships.author.id:${encodedAuthorId},from_publication_date:${fromYear}-01-01&sort=publication_year:desc&per-page=10${mailtoQuery}`;
