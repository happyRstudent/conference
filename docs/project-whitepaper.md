...
1. OpenAlex：检索作者、机构、国家、论文数、被引数、H-index、概念标签等。
...
1. 构造最多 3 个查询变体。
2. 优先搜索 `authors`。
3. 若作者结果不足，再搜索相关 `works`，从论文作者信息中补充候选人。
4. 根据 `scope` 限定地区：
   - `domestic` 只保留 `CN`
   - `international` 排除 `CN`
...
```text
总分 = 关键词匹配分 + 被引分 + H-index 分 + 学术活跃度分 + 青年优先加分
```

具体规则如下：

1. 关键词匹配分：`matchedKeywords.length * 10`，上限 55 分。
2. 被引分：`log10(citedByCount + 1) * 8`，上限 20 分。
3. H-index 分：`hIndex * 0.45`，上限 15 分。
4. 学术活跃度分：`worksCount * 0.08`，上限 10 分。
5. 青年优先加分：若 `preferYoungScholar = true` 且命中青年规则，加 3 分。

最终结果四舍五入为整数。

### 6.2 关键词匹配规则

`matchedKeywords` 来自议题关键词集合与学者概念标签、近五年论文标题分词之间的交集，最多保留 5 个。
...
LLM 的职责不是找新的人，而是在现有候选池中完成二次排序与说明优化，且受到明确约束：

1. 只能从提供的候选人中选择，禁止编造人物。
2. 优先判断是否与议题“直接贴合”。
3. 泛相关候选人不能优先入选。
4. 在相关性相近时，才允许适度偏向青年学者。

LLM 输出后，系统会把被选中的候选人分数提升到不低于 `88 - index * 4` 的水平，用于体现复核优先级，但不会替换其真实身份信息。
